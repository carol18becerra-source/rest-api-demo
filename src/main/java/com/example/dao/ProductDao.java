...
public interface ProductDao extends JpaRepository<Product, Integer> {
     ...
     * que recuperen la presentacion en una sola consulta, que es mas eficiente, no
     * no?
     * Bueno, no es mas eficiente pero si mas rapido estara disponible la
     * presentacion
     * para cada uno de los productos. Mas eficiente no es porque se consumen mas
     * recursos
     * , puesto que con el patron LAZY (perezoso) se trae la entidad principal e
     * hibernate
     * no trae las entidades relacionadas hasta su primer uso, y se requiere mas
     * velocidad
     ...
     * por el nombre, por ejemplo, porque podria ser por cualquier otro campo de la
     * entidad
     * o por varios campos inclusive (stock, por ejemplo)
     * 3- Dado el id del producto, recupere el producto con su presentacion
     * correspondiente.
     * 
     * * Para implementar los metodos anteriores vamos a hacer uso de HQL/JPQL, que
     * son
     * lenguajes basados en SQL pero orientados a las entidades y no a las tablas de
     * la
    ...
    @Query(value = "select p from Product p left join p.presentation", countQuery = "select count(p) from Product p left join p.presentation")
